...
def reset_game(updatable, drawable, asteroids, shots, player, game_info, asteroid_field):
    asteroids.empty()
    shots.empty()
    ...
    while True:
        ...
        # Überprüfung auf Tastendruck
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN:  # Spiel komplett neu starten
                # Setze alles zurück, einschließlich der Lebensanzahl auf 3
                        reset_game(updatable, drawable, asteroids, shots, player, game_info, asteroid_field)
                        game_info.lives = 3  # Setze die Leben auf 3 zurück
                        start_ticks = pygame.time.get_ticks()  # Setze die Startzeit zurück
                        waiting_for_input = False  # Verlasse die Eingabewarteschleife, um das Spiel neu zu starten
