...
# Blende den Spieler aus, wenn er unverwundbar ist und der Alpha-Wert 0 (unsichtbar) ist
                if invincibility_time > 0:
    # Ändere den Alpha-Wert des Spielerbildes
                    player.alpha_value = alpha_value  # Setze den alpha_value des Spielers

                # Blinken des Spielers, wenn er unverwundbar ist
                if blink_time > 0:
                    blink_time -= dt * 1000
                    if blink_time <= 0:
                        # Toggle der Sichtbarkeit des Spielers durch Ändern des Alpha-Kanals
                        alpha_value = 255 if alpha_value == 0 else 0  # Wechsel zwischen voll sichtbar (255) und unsichtbar (0)
            ...
            if invincibility_time > 0:
                # Ändere den Alpha-Wert des Spielerbildes
                player.image.set_alpha(alpha_value)
